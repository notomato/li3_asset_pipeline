...
class Locator
{
     ...
     * @param array $processors List of filter names, that could be used
     */
    public function __construct(array $paths = array(), array $processors = array())
    {
    ...
    protected function locate($input, $type, $include = false, $from = null)
    {
        ...
        foreach ($this->paths as $path) {
            ...
            // preprocessor asset
			// We do a regex search in the files 
			$fullpath = $path.'/'.$type.'/'.$fileWithExtension;
			...
			if (is_dir($path)) {
				foreach ($assetFiles->in($path) as $file) {
					return $this->processAssetsDirectives(array(array(
						'include' => false,
						'file'    => str_replace($path.'/', '', $file->getPathname()),
						'filter'  => array_keys(Asset::getProcessors($file->getPathname())),
						'root'    => $path
					)), $type);
				}
			}
